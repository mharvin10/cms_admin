        ...
        <v-flex xs12 lg8 offset-lg2>
          ...
          <v-layout column class="pa-1">
            <v-form v-model="form.valid" ref="createAnnouncementForm">
              <div class="title mt-2 mb-3" :class="$store.state.style.text.color.formLabel">Edit Announcement</div>
              <v-card raised>
                <v-container fluid>
                  <v-text-field
                    label="Title"
                  ...
                  <v-layout align-center class="subheading mt-4 mb-2" :class="$store.state.style.text.color.formLabel">
                    <span>Don't show this announcement?</span>
                    <v-checkbox
...
export default {
  ...
  methods: {
    ...
    update (announcementId) {
      this.axios.patch('/admin/announcement/' + announcementId, this.form.data)
        .then(response => {
