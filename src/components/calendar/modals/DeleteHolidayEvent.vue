      ...
      <v-card-text style="height: 130px;">
        <v-layout row wrap fill-height>
          <v-flex xs12 fill-height>
            <div class="ml-2">Select the checkbox below to confirm that you understand the impact of deleting this holiday/event:</div>
            <!-- form -->
            <form ref="confirmDelete">
              <v-layout align-center class="mt-2 ml-2">
                <v-checkbox
                  ...
                  class="shrink mr-2"
                ></v-checkbox>
                <span class="error--text">Delete this holiday/event</span>
              </v-layout>
            </form>
            <!-- end form -->
          </v-flex>
...
<script>
  export default {
    ...
    methods: {
      ...
      confirmDelete (calendarDateId) {
        this.axios.delete('admin/calendar-date/' + calendarDateId)
          .then(response => {
            this.props.show = false
            this.$emit('holidayEventDeleted')
          })
          .catch(error => {
