  ...
  <v-dialog v-model="props.show" fullscreen transition="dialog-bottom-transition" :overlay="false">
    <v-card flat>
      <v-toolbar dark color="primary" flat>
        ...
        <v-spacer></v-spacer>
        <v-btn flat small @click="save(props.data.id)">Save</v-btn>
      ...
      <v-layout row wrap py-3>
        <v-flex xs12 lg8 offset-lg2>
          ...
          <v-layout column class="pa-1">
            <v-form v-model="form.valid" ref="editHolidayEventForm">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      save (calendarDateId) {
        this.axios.patch('/admin/calendar-date/' + calendarDateId, this.form.data)
          .then(response => {
            // console.log(response.data)
            this.props.show = false
            this.$emit('holidayEventEdited')
          })
          .catch(error => {
