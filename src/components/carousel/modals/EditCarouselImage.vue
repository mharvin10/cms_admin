  ...
  <v-dialog v-model="props.show" fullscreen transition="dialog-bottom-transition" :overlay="false">
    <v-card flat>
      <v-toolbar dark color="primary" flat>
        ...
        <v-spacer></v-spacer>
        <v-btn flat small @click="update(props.data.id)">Update</v-btn>
        ...
        <v-flex xs12 lg8 offset-lg2>
          ...
          <v-layout column class="pa-1">
            <v-form v-model="form.valid" ref="addCarouselImageForm">
              <div class="title mt-2 mb-3" :class="$store.state.style.text.color.formLabel">Update carousel image</div>
              <v-card raised>
                <v-container fluid>
                  <v-text-field
                    label="Title"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      update (carouselImageId) {
        this.axios.patch('/admin/carousel-image/' + carouselImageId, this.form.data)
          .then(response => {
            // console.log(response.data)
            this.props.show = false
            this.$emit('carouselImageEdited')
          })
          .catch(error => {
            console.log(error.response.data)
