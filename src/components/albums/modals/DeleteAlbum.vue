      ...
      <v-card-text style="height: 150px;">
        <v-layout row wrap fill-height>
          <v-flex xs12 fill-height>
            <div class="ml-2">Select the checkbox below to confirm that you understand the impact of deleting this album:</div>
            <!-- form -->
            <form ref="confirmDelete">
              <v-layout align-center class="mt-2 ml-2">
                <v-checkbox
                  ...
                  class="shrink mr-2"
                ></v-checkbox>
                <span class="error--text">Delete this album</span>
              </v-layout>
              <v-layout align-center class="ml-2">
                <v-checkbox
                  v-model="form.deletePhotos"
                  color="primary"
                  hide-details 
                  class="shrink mr-2"
                  :disabled="!form.confirm"
                ></v-checkbox>
                <span :class="!form.confirm ? 'grey--text text--lighten-1' : 'error--text'">Delete all photos from this album in the storage</span>
              </v-layout>
            </form>
            <!-- end form -->
          </v-flex>
  ...
  export default {
    ...
    methods: {
      ...
      confirmDelete (albumId) {
        this.axios.post('admin/album/' + albumId + '/delete', this.form)
          .then(response => {
            this.props.show = false
