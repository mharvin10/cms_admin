      ...
      <v-layout row wrap py-3>
        <v-flex xs12 lg8 offset-lg2>
          ...
          <v-layout column class="pa-1">
            <v-form v-model="form.valid" ref="createNewsForm">
              <div class="title mt-2 mb-3" :class="$store.state.style.text.color.formLabel">Create News</div>
              <v-card raised>
                <v-container fluid>
                  <v-text-field
                    label="News title"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      save () {
        this.axios.post('/admin/news', this.form.data)
          .then(response => {
            // console.log(response.data)
